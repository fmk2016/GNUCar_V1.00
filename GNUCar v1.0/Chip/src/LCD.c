 ...
 * 注意  ：字符串显示方向为横 已测试
******************************************************************/
u8 LCD_Char(Site_t site, const u8 ascii, u16 Color, u16 bkColor)
{
...
void LCD_Str(Site_t site, u8 *Str, u16 Color, u16 bkColor)
{
#define MAX_CHAR_POSX (LCD_W-8)
#define MAX_CHAR_POSY (LCD_H-16)
    while(*Str != '\0')
...
void LCD_Str_CH(Site_t site, const u8 *str  , u16 Color , u16 bkColor) 		//显示16*16汉字字符串
{
    while(*str != '\0')
    {
        if(site.x > (LCD_W - 16))
 ...
 * 注意  ：字符串显示方向为横 已测试
******************************************************************/
void LCD_Str_R(Site_t site, const u8 *str, u16 Color, u16 bkColor) //右对齐，超过35位，仅显示最后35位
{
    u16 i = 1, strlen = 0;
    Site_t sitetemp;
    sitetemp.y = site.y;
...
void LCD_Num_C(Site_t site, u32 num, u16 Color, u16 bkColor)
{
    u32 res = num;
    u8 t = 0;
    Site_t sitetemp;
...
void LCD_Img_RGB565	(Site_t site, Size_t size, u16 *img)
{
    u32 n, temp;
...
void LCD_Img_RGB565_Z(Site_t site, Size_t size, u16 *img, Size_t imgsize)
{
...
void LCD_Img_Binary(Site_t site, Size_t size, u16 *img)
{
...
************************************************************************/
void LCD_Char_CH(Site_t site, const u8 *str, u16 Color, u16 bkColor)
{
