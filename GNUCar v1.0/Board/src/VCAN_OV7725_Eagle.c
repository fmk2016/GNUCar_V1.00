...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTA_B1_IN, (void *)ov7725_eagle_img_buff, PTA7, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
    ...
    port_init(PTA7, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK

    port_init(PTA16, ALT1 | IRQ_RISING  | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波

}

/*!
 *  @brief      鹰眼ov7725 场中断服务函数
 *  @since      v5.0
 */
...
#else
        PORTA_ISFR = 1 <<  PT7;            //清空PCLK标志位
        dma_repeat(CAMERA_DMA_CH, (void *)&PTA_B1_IN, (void *)ov7725_eagle_img_buff,CAMERA_DMA_NUM);
#endif
    }
    else                                        //图像采集错误
    {
